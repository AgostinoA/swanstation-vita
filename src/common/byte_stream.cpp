...
public:
  ...
  virtual bool SeekAbsolute(u64 Offset) override
  {
    ...
    if (fseek(m_pFile, static_cast<off_t>(Offset), SEEK_SET) != 0)
    {
      m_errorState = true;
      return false;
    }
  ...
  virtual bool SeekRelative(s64 Offset) override
  {
    ...
    if (fseek(m_pFile, static_cast<off_t>(Offset), SEEK_CUR) != 0)
    {
      m_errorState = true;
      return false;
    }
  ...
  virtual u64 GetPosition() const override { return static_cast<u64>(ftell(m_pFile)); }

  virtual u64 GetSize() const override
  {
    off_t OldPos = ftell(m_pFile);
    fseek(m_pFile, 0, SEEK_END);
    off_t Size = ftell(m_pFile);
